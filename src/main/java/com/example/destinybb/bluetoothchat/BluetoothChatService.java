...
//1.在onStart()中，檢查如果ConnectThread和ConnectedThread存在，則將他們關掉。
  ...
  //2.啟動一個AcceptThread(現在的流程是在藍芽開啟中的狀態，開啟了一個AcceptThread待命)。這個AcceptThread存在的目的，是因為程式先假設每臺裝置都有可能想要跟它做藍芽連線。
...
isServer = false;
    ...
    /**
     * BluetoothServerSocket是一個開放式的server socket，用來監聽任何傳進來的請求(原理類似TCP ServerSocket)。為了讓2隻Android devices能夠連線，
     * 其中一隻裝置必須開啟server socket。當遠端的藍芽裝置向手上這隻裝備請求連線後，這隻裝置上的BluetoothServerSocket會回傳一個accepted的 BluetoothSocket給呼叫那一方。
     */
